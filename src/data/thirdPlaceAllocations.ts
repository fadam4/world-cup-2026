// FIFA World Cup 2026 - Third Place Team Allocations
// All 495 possible combinations from FIFA Annex C
// This determines which third-place team plays in which Round of 32 match
// based on which 8 groups have the best third-place teams

export interface ThirdPlaceAllocationMap {
  [qualifyingGroups: string]: {
    [groupWinner: string]: string; // groupWinner -> thirdPlaceGroup
  };
}

// Group winners that play against third-place teams
export const GROUP_WINNERS_VS_THIRD_PLACE = ['A', 'B', 'D', 'E', 'G', 'I', 'K', 'L'] as const;

// All 495 possible third-place allocations
export const THIRD_PLACE_ALLOCATIONS: ThirdPlaceAllocationMap = {
...
// Helper function to get third-place allocation
export function getThirdPlaceAllocationFromFIFA(qualifyingThirdPlaceGroups: string[]): Map<string, string> {
  const key = [...qualifyingThirdPlaceGroups].sort().join('');
  const allocation = THIRD_PLACE_ALLOCATIONS[key];
  
  if (!allocation) {
    throw new Error(`No allocation found for combination: ${key}`);
  }
  
  const result = new Map<string, string>();
  for (const [groupWinner, thirdPlaceGroup] of Object.entries(allocation)) {
    result.set(groupWinner, thirdPlaceGroup);
  }
